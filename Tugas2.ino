...
const char* ssid = "CONTAINER CAFE";          // SSID WiFi
const char* password = "UMM.1964";    // Password WiFi
const char* serverUrl = "http://172.16.98.82/Tugas2/save_data.php";
...
void loop() {
    ...
    if (arah != "stabil" && WiFi.status() == WL_CONNECTED) {
        ...
        url += "?arah=" + arah;
        url += "&angka_sumbu=" + String(angka_sumbu);  // Tambahkan parameter ke URL

        HTTPClient http;
        http.begin(url);

        int httpResponseCode = http.GET();  // Gunakan metode GET
    ...
    delay(3000);  // Tunggu 1 detik sebelum membaca ulang
}
